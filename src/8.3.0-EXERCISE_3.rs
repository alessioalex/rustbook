...
impl Company {
    ...
    fn print_employees_by_department(&self, department: &str) {
        match self.get_employees_by_department(department) {
            Some(employees) => {
                // keep original source as it is, maybe the initial ordering matters
                let mut employees_copy = employees.clone();
                employees_copy.sort();
                println!("{} employees: {}.", department, employees_copy.join(", "));
            }
            None => println!("The sales department has no employees."),
        }
    }

    fn print_all_employees(&self) {
        println!();
        println!("~~~ Printing company employees ~~~");
        for department in self.departments.keys() {
            self.print_employees_by_department(department);
        }
        println!("~~~ . ~~~");
    }
}
...
fn main() {
    ...
    let text = "Add Amir to Sales";
    company.process(text);
    let text = "Add Victoria to Sales";
    company.process(text);

    company.print_employees_by_department("Sales");
